...
export interface MemoryStoreConfig {
  // Здесь можно добавить конфигурацию для разных типов хранилищ
  // например, для Redis или MongoDB в будущем
  type?: 'memory' | 'redis' | 'mongo';
  ttl?: number; // время жизни сессии в миллисекундах
}
...
export class MemoryStore implements IMemoryStore {
  ...
  constructor(config: MemoryStoreConfig = {}) {
    this.config = {
      type: 'memory',
      ttl: 7200000, // 2 часа по умолчанию
      ...config
    };
  }
  ...
  cleanup(): void {
    const now = Date.now();
    const ttl = this.config.ttl || 7200000;
